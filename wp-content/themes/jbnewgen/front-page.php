...
<section id="hero" class="relative isolate -mt-16 flex min-h-svh items-center overflow-hidden">
	...
	if ( file_exists( get_template_directory() . '/assets/hero.mp4' ) ) :
		?>
		<video class="hero-video absolute inset-0 h-full w-full object-cover object-center" autoplay muted loop playsinline preload="metadata" aria-hidden="true" tabindex="-1">
		...
		<div data-reveal data-reveal-delay="150">
			<figure class="relative overflow-hidden rounded-3xl bg-ink-950 text-white shadow-[0_40px_80px_-44px_rgba(8,21,39,0.6)]">
				...
				<div class="relative p-8 sm:p-10">
					...
					<div class="mt-8 flex items-center justify-end border-t border-white/10 pb-2 pt-10">
...
<section class="relative overflow-hidden bg-ink-950 py-20 text-white sm:py-28">
	...
	<div class="relative mx-auto max-w-7xl px-5 sm:px-8">
		...
		<?php
		// The React version rotated through one pillar at a time on a 6s timer.
		// Showing them all at once is the honest static equivalent: the same
		// information, no JavaScript, and nothing hidden from a crawler.
		?>
		<div class="mt-14 grid gap-6 md:grid-cols-2 lg:grid-cols-4">
			<?php foreach ( $jb_pillars as $jb_i => $jb_pillar ) : ?>
				...
				?>
				<a href="<?php echo esc_url( $jb_pillar['url'] ); ?>" class="group relative flex flex-col overflow-hidden rounded-2xl border border-white/15 bg-white/[0.07] p-5 transition-colors duration-300 hover:border-white/30 hover:bg-white/[0.1]" data-reveal data-reveal-delay="<?php echo (int) ( $jb_i * 80 ); ?>">
					...
					<h3 class="mt-4 text-lg font-bold leading-tight text-white"><?php echo esc_html( $jb_full ? $jb_full : $jb_pillar['short'] ); ?></h3>
					<?php if ( $jb_subtext || $jb_blurb ) : ?>
						<p class="mt-3 flex-1 text-pretty text-[0.9rem] leading-relaxed text-ink-300"><?php echo esc_html( $jb_subtext ? $jb_subtext : $jb_blurb ); ?></p>
					<?php endif; ?>
					<span class="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-flame-400">
						<?php esc_html_e( 'Explore', 'jbnewgen' ); ?> <?php echo esc_html( $jb_pillar['short'] ); ?>
						<?php jbnewgen_icon( 'arrowRight', 16, 'transition-transform group-hover:translate-x-0.5' ); ?>
					</span>
				</a>
			<?php endforeach; ?>
		</div>
